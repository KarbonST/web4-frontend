...
const ThemeToggle: React.FC = () => {
    ...
    const label = theme === 'dark' ? 'Темная' : 'Светлая';

    return (
        <button
            type="button"
            className={`model-toggle model-toggle_${theme}`}
            onClick={toggle}
            aria-label="Переключить тему"
        >
            <svg
                className="model-toggle__icon"
                width="36"
                height="32"
                viewBox="0 0 36 32"
                fill="none"
            ...
            >
                ...
                />
                <defs>
                    <linearGradient id="paint0" x1="4" y1="5" x2="18" y2="25" gradientUnits="userSpaceOnUse">
                        <stop stopColor="currentColor" stopOpacity="0.8" />
                        <stop offset="1" stopColor="currentColor" stopOpacity="0.4" />
                    </linearGradient>
                    <linearGradient id="paint1" x1="32" y1="5" x2="18" y2="25" gradientUnits="userSpaceOnUse">
                        <stop stopColor="currentColor" stopOpacity="0.8" />
                        <stop offset="1" stopColor="currentColor" stopOpacity="0.4" />
                    </linearGradient>
                </defs>
            </svg>
            <span className="model-toggle__label" aria-hidden>{label}</span>
        </button>
    );
};
